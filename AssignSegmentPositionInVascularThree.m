...
%%
nGroups = length(unique(im2.nodeGrp));
disp(['Number of groups is ' num2str(nGroups) ]);
disp('Grp. number      num. elements');
for i=1:nGroups,
    grpElements(i) = length(find(im2.nodeGrp==i));
...
totNumSegments = length(im2.segDiam); % number of segments
foo1 = im2.nodeGrp(im2.segEndNodes(:,1));
foo2 = im2.nodeGrp(im2.segEndNodes(:,2));
...
%idxArt = find(im2.segVesType==1);
%[foo, maxArtIdx] = sort(im2.segDiam(idxArt),'descend');
%maxArtIdx = idxArt(maxArtIdx); % descending indexes of arterial segments
%%
segEndNodes = im2.segEndNodes(segLst,:); % use only segments that belong to group grp
...
%                 lst = find(im2.segEndNodes(:,1) == node1);
%                 lst2 = find(im2.segEndNodes(:,2) == node2);
%                 c = intersect(lst, lst2);
%                 if ~isempty(c),
%                     segConnectivityMartix(ii,jj)=im2.segLen_um(c(1));
%                     segConnectivityMartix(jj,ii)=segConnectivityMartix(ii,jj);
%                 else
%                     lst = find(im2.segEndNodes(:,1) == node2);
%                     lst2 = find(im2.segEndNodes(:,2) == node1);
%                     c = intersect(lst, lst2);
%                     if ~isempty(c),
%                         segConnectivityMartix(ii,jj)=im2.segLen_um(c(1));
%                         segConnectivityMartix(jj,ii)=segConnectivityMartix(ii,jj);
...
segLen_um = im2.segLen_um(segLst);
...
for ii=1:size(pialseg,1), % find end nodes of pial segments and convert them based on lookuptable
    pialseg(ii,4) = find(lookuptable == im2.segEndNodes(pialseg(ii,1),1));
    pialseg(ii,5) = find(lookuptable == im2.segEndNodes(pialseg(ii,1),2));
end;
%%
...
for ii=1:numSegments,
    ...
    segBR_Art(ii,6) = im2.segDiam(segLst(ii)); % segment diameters
    segnode1 = find(lookuptable == im2.segEndNodes(segLst(ii),1));
    segnode2 = find(lookuptable == im2.segEndNodes(segLst(ii),2));
...
for ii = 1:length(im2.PO2pts.PO2),
    po2Seg = im2.PO2pts.seg(ii);
    po2_BRorder(ii) = segBR_Art(find(segBR_Art(:,1)==po2Seg),2);
    po2_PO2(ii) = im2.PO2pts.PO2(ii);
end;
...
for ii = 1:length(im2.PO2pts.PO2),
    ...
    if( (im2.segVesType(po2Seg) == 1) || (im2.segVesType(po2Seg) ==2) ), % if arterial or cappilary
        n=n+1;
        po2_BRorder(n) = segBR_Art(find(segBR_Art(:,1)==po2Seg),2);
        po2_PO2(n) = im2.PO2pts.PO2(ii);
...
for ii = 1:length(im2.PO2pts.PO2),
    ...
    %if (segBR_Art(find(segBR_Art(:,1)==po2Seg),4) > 4), %try to exclude
    %venules, veins, everything labeled as venous side
    if( (im2.segVesType(po2Seg) == 3) || (im2.segVesType(po2Seg) ==2) ), % if vein or cappilary
        n=n+1;
        po2_BRorder(n) = segBR_Art(find(segBR_Art(:,1)==po2Seg),2);
        po2_PO2(n) = im2.PO2pts.PO2(ii);
        po2_SO2(n) = (po2_PO2(n))^hillC / ( (po2_PO2(n))^hillC + p50^hillC  ); 
