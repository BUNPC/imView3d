% Here we use OCT priors in the computation of Flow. We also itterate in
% case of high velocities.
%
% Created on 4/27/2012 by L. Gagnon
%
%
% TO DO
...
function imView3d_flowCircuitEq_OCT( )
global im
...
OCTsegID=im.OCTflow.seg;
nodeOCTflow=im.OCTflow.nodeOCT;
...
%%%%%%%%%%%%%%%%%%%%%%%%
% map OCT priors
OCTprior=zeros(nSegs,1);
for jj=1:length(OCTsegID)
    
    segID=OCTsegID(jj);
    
    %check if we need to change the sign
    %convention is that flow is positive from p1 to p2 in nSegNodes
    pos1=nodePos(segEndNodes(segID,1),:);
    pos2=nodePos(segEndNodes(segID,2),:);
    
    segOCTflow=nodeOCTflow(segEndNodes(segID,1));
    if segOCTflow==0
        segOCTflow=nodeOCTflow(segEndNodes(segID,2));
    end
    
    %determine if flow is going from 1 to 2 or verse versa
    %OCT flow are positive for downward direction and negative for
    %upward direction
    if pos1(3)>pos2(3) && segOCTflow>0 %flow going downward from 1 to 2
        theSign=1;
    elseif pos1(3)>pos2(3) && segOCTflow<0 %flow going upward from 2 to 1
        theSign=-1;
    elseif pos1(3)<pos2(3) && segOCTflow>0 %flow going downward from 2 to 1
        theSign=-1;
    elseif pos1(3)<pos2(3) && segOCTflow<0 %flow going upward from 1 to 2
        theSign=1;
    end
    
    %OCT flow is in nanoliters/min -> convert to um^3/sec
    % 1 nL/min = 1.66667E4 um^3/sec (or 10^6/60 um^3/sec)
    OCTprior(OCTsegID(jj))=theSign*(1e6)/60*abs(segOCTflow);
...
% else
    ...
    p1map=zeros(nSegs,1);
    p2map=zeros(nSegs,1);
    Mr = zeros(nSegs,nSegNodes+nSegs);
    for iS = 1:nSegs
        p1 = segEndNodes(iS,1);
        p2 = segEndNodes(iS,2);
        p1map(iS)=p1;
        ...
        %check if OCT value for this segment
        if OCTprior(iS)~=0
            Mr(iS,nSegNodes+iS)=1;
    ...
    for ii=1:nSegNodes
        ...
        if nSegB(ii)>1   % flow conservation
            ...
            for jj=1:length(lstR)
                ...
                %check if OCT value for this segment
                if OCTprior(lstR(jj))~=0
                    if lstC(jj)==1
                        y(ii)=y(ii)-OCTprior(lstR(jj));
                    else
                        y(ii)=y(ii)+OCTprior(lstR(jj));
                    end
                else
        ...
        elseif nSegB(ii)==1  % use a BC
            ...
            if nodeBCType(segNodeMap(ii))==1 || nodeBCType(segNodeMap(ii))==3  % pressure BC
                ...
                if OCTprior(lstR)~=0
                    p1 = edges(lstR,lstC);
                    p2 = edges(lstR,mod(lstC,2)+1);    
                    M(ii,p1)=1/eR(lstR);
                    M(ii,p2)=-1/eR(lstR);
                    if lstC==1
                        y(iNode)=OCTprior(lstR); %noise
                    else
                        y(iNode)=-OCTprior(lstR); %noise
                    end
                    
                    
%                     %this one was causing bad conditioning of the M matrix
%                     M(ii,p1)=1;
%                     M(ii,p2)=-1;
%                     if lstC==1
%                         y(iNode)=eR(lstR)*OCTprior(lstR); %noise
%                     else
%                         y(iNode)=eR(lstR)*-OCTprior(lstR); %noise
%                     end
%                     
                else %use Pressupre BC
                    ...
                    elseif nodeBCType(segNodeMap(ii))==3 %from literature
                        M(ii,ii) = 1;
                        y(ii) = 1/2*getPressure( segDiam(lstR), segVesType(lstR) ); %Can add scaling factor here
                    end
                    
                    %keep track of highest Pressure BC
                    if y(ii)>maxP_BC;
                        maxP_BC=y(ii);
                        maxP_BCidx=ii;
                    end
            ...
            elseif nodeBCType(segNodeMap(ii))==2 || nodeBCType(segNodeMap(ii))==4   % velocity BC
                ...
                if nodeBCType(segNodeMap(ii))==2 %user defined
                    ...
                    %No need to worry about OCT here. Why would someone set
                    %the velocity in a segment for which OCT is available?
                ...
                elseif nodeBCType(segNodeMap(ii))==4 %from litterature. How do we set the sign ?????
                    ...
                    display('Velocity BC based on literature values is not available at the moment')
...
%                     flow = vel * 3.14159 * (segDiam(lstR)/2)^2;
                ...
                display(sprintf('BC not specified for node %d',segEndNodes(ii)))
...
while ~isempty(tooFast) && cpt<=MaxNLoop
    ...
    F = Mr(:,[lstSegMap' (nSegNodes+1):(nSegNodes+nSegs)]) * [P(lstSegMap)'; OCTprior];
...
hwait = waitbar(0,'Interpolating pressure for segments with OCT');

%reinterpolate pressure in nodes from F=Mr*P
%fix pressure in one node to avoid offsets

Mint=zeros(1,size(Mr,2));
Mint(1,maxP_BCidx)=1;
Mint=[Mint; Mr];
P = (Mint'*Mint)\Mint'*[maxP_BC; F];
close(hwait)
