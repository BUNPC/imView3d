function imView3d_erodeCenterLine( Ithresh, ROI )
global im
...
[nIx,nIy,nIz] = size(I);
nN = size(im.nodePos(ROI,:),1);
lstIdx1 = min(round(im.nodePos(ROI,2)),nIy) + (min(round(im.nodePos(ROI,1)),nIx)-1)*nIy + (min(round(im.nodePos(ROI,3)),nIz)-1)*nIx*nIy;
lstIdx=ROI(lstIdx1);
