...
if ch==6
    ...
    for iii= 1:length(nLst) %1:nN
        ...
        proceedFlag=0; % filter nodes on diamter thresshold
        if isfield(im,'straightThreshFlag')
            if im.straightThreshFlag==1
                if im.nodeDiam(ii)>=im.setDiamStraightThresh
                    proceedFlag=1;
                end
            else
                proceedFlag=1;
        ...
        proceedFlag2=0; % select nodes inside Z range
        if isfield(im,'straightZRangeFlag')
            if im.straightZRangeFlag==1
                if ((im.nodePos(ii,3)>=im.setZRangeStraight(1,1)) && (im.nodePos(ii,3)<=im.setZRangeStraight(2,1)));
                    proceedFlag2=1;
                end
            else
                proceedFlag2=1;
        ...
        proceedFlag3=0; % select nodes inside XY range
        if isfield(im,'selectXYLimitsStraighteningFlag')
            if im.selectXYLimitsStraighteningFlag==1
                if (((im.nodePos(ii,1)>=im.selectXYLimitsStraightening(1,1)) && (im.nodePos(ii,1)<=im.selectXYLimitsStraightening(2,1))) && ((im.nodePos(ii,2)>=im.selectXYLimitsStraightening(3,1)) && (im.nodePos(ii,2)<=im.selectXYLimitsStraightening(4,1))));
                    proceedFlag3=1;
                end
            else
                proceedFlag3=1;
